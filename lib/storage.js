...
// Ensure b1-passe-compose-etre sits immediately after b1-avoir-conjugations (PN1-04).
// Runs once on first load after deploy; saves back to Redis so it never runs again.
function applyMigrations(state) {
  ...
  if (etreIdx === -1 || avoirIdx === -1 || etreIdx === avoirIdx + 1) return { state, changed: false };
